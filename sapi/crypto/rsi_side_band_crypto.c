...
int32_t rsi_sha(uint8_t sha_mode, uint8_t *msg, uint16_t msg_length, uint8_t *digest)
{
  ...
  status = rsi_check_and_update_cmd_state(CRYPTO_CMD, IN_USE);
  ...
  if (status == RSI_SUCCESS) {
    ...
    if (sha_mode == SHA_1) {
      ...
      rsi_check_and_update_cmd_state(CRYPTO_CMD, ALLOW);
      SL_PRINTF(SL_SHA_INVALID_PARAM_2, CRYPTO, LOG_ERROR);
      return RSI_ERROR_INVALID_PARAM;
    }

    // Allocate command buffer  from wlan pool
    pkt = rsi_pkt_alloc(&rsi_crypto_cb->crypto_tx_pool);
    ...
    if (digest != NULL) {
      ...
      rsi_crypto_cb->crypto_buffer = digest;

      // Length of the buffer provided by user
      rsi_crypto_cb->crypto_buffer_length = digest_len;
    }
    ...
    status = rsi_driver_crypto_send_cmd(RSI_COMMON_REQ_ENCRYPT_CRYPTO, pkt);

    // Wait on common semaphore
    rsi_wait_on_crypto_semaphore(&rsi_driver_cb_non_rom->crypto_cmd_sem, RSI_CRYPTO_RESPONSE_WAIT_TIME);

    // Get common command response stattus
    status = rsi_crypto_get_status();
 ...
 */
  ...
  /** @endcond */
  {
    ...
    if (status == RSI_SUCCESS) {
      ...
      if (output != NULL) {
        // Attach the buffer given by user
        rsi_crypto_cb->crypto_buffer = output;

        // Length of the buffer provided by user
        rsi_crypto_cb->crypto_buffer_length = msg_length;
      }
      ...
      // Fill chunk length
      aes->current_chunk_length = chunk_len;
      ...
      // Wrap IV
      // aes->key_info.key_detail.key_spec.wrap_iv = wrap_iv;
      // Memset before filling
      ...
      // Using host descriptor to set payload length
      send_size = sizeof(rsi_aes_req_t);
 ...
 */
                             ...
                             uint8_t * exp_result)
  {
    ...
    if (status == RSI_SUCCESS) {
      ...
      if (exp_result != NULL) {
        // Attach the buffer given by user
        rsi_crypto_cb->crypto_buffer = exp_result;

        // Length of the buffer provided by user
        rsi_crypto_cb->crypto_buffer_length = DH_MAX_OUTPUT_SIZE;
      }
 ...
 */
                                        ...
                                        uint8_t * rz)
  {
    ...
    if (status == RSI_SUCCESS) {
      ...
      if (result != NULL) {
        // Attach the buffer given by user
        rsi_crypto_cb->crypto_buffer = result;

        // Length of the buffer provided by user
        rsi_crypto_cb->crypto_buffer_length = ECDH_MAX_OUTPUT_SIZE;
      }
...
#endif
  {
    ...
    if (status == RSI_SUCCESS) {
      ...
      if (output != NULL) {
        ...
        //! length of the buffer provided by user
        rsi_crypto_cb->crypto_buffer_length = msg_length + TAG_SIZE;
      }
      ...
      //! Using host descriptor to set payload length
      send_size = sizeof(rsi_gcm_req_t);
 ...
 */
  ...
  /*==============================================*/
  /**
 * @fn          int32_t rsi_driver_crypto_send_cmd(rsi_crypto_cmd_request_t cmd, rsi_pkt_t *pkt)
 ...
 */
  ...
  {
    LOG_PRINT("crypto_send_cmd\r\n");
    uint16_t payload_size = 0;
    int32_t status        = RSI_SUCCESS;
    // Get crypto control block structure pointer
    rsi_crypto_cb_t *rsi_crypto_cb = rsi_driver_cb->crypto_cb;
    // Get host descriptor pointer
    uint8_t *host_desc = (pkt->desc);

    // Enqueue packet to crypto TX queue
    rsi_enqueue_pkt(&rsi_driver_cb->crypto_tx_q, pkt);

    // Set CRYPTO packet pending event
    rsi_set_event(RSI_CRYPTO_TX_EVENT);
  ...
  /*==============================================*/
  /**
 * @fn          rsi_error_t rsi_wait_on_crypto_semaphore(rsi_semaphore_handle_t *semaphore, uint32_t timeout_ms )
 * @brief       Wait for crypto semaphore
 ...
 */
  ...
  rsi_error_t rsi_wait_on_crypto_semaphore(rsi_semaphore_handle_t * semaphore, uint32_t timeout_ms)
  {
    ...
    if (rsi_semaphore_wait(semaphore, timeout_ms) != RSI_ERROR_NONE) {
      rsi_crypto_set_status(RSI_ERROR_RESPONSE_TIMEOUT);
#ifndef RSI_WAIT_TIMEOUT_EVENT_HANDLE_TIMER_DISABLE
      if (rsi_driver_cb_non_rom->rsi_wait_timeout_handler_error_cb != NULL) {
        rsi_driver_cb_non_rom->rsi_wait_timeout_handler_error_cb(RSI_ERROR_RESPONSE_TIMEOUT, CRYPTO_CMD);
      }
#endif
  ...
  /*==============================================*/
  /**
 * @brief       Set the crypto block status. This is a non-blocking API.
 ...
 */
  ...
  {
    rsi_driver_cb->crypto_cb->status = status;
  }
...
*/
  /*==============================================*/
  /**
 * @brief       Return crypto block status. This is a non-blocking API.
 * @param[in]   Void
 * @return      0              -  Success \n
 *              Non-Zero Value -  Failure
 */
  ...
  {
    return rsi_driver_cb->crypto_cb->status;
  }
