...
#endif
rsi_semaphore_handle_t ble_main_task_sem, ble_peripheral_conn_sem, bt_app_sem, wlan_app_sem, bt_inquiry_sem,
  ble_scan_sem;
...
// rsi_wireless_driver_task is creating in rsi_common_app_task only for EFM platform
    ...
    if (RSI_BLE_MAX_NBR_PERIPHERALS > 0) {
      status = rsi_semaphore_create(&ble_peripheral_conn_sem, 0);
      if (status != RSI_ERROR_NONE) {
