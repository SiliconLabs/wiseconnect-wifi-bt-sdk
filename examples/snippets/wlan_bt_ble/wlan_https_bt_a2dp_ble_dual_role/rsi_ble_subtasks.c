 ...
 *
 *  @section Description  This application connects as a Central with l2cap connection.
 *
 */
...
void rsi_ble_task_on_conn(void *parameters)
{
  ...
  if (l_conn_id < RSI_BLE_MAX_NBR_PERIPHERALS) {
    ...
    switch (event_id) {
      ...
      case RSI_BLE_CONN_EVENT: {
        ...
        if (l_conn_id >= RSI_BLE_MAX_NBR_PERIPHERALS) {
          ...
          if ((num_of_conn_centrals < RSI_BLE_MAX_NBR_CENTRALS) & !CHK_BIT(rsi_ble_states_bitmap, RSI_ADV_STATE)) {
          ...
          if ((num_of_conn_peripherals < RSI_BLE_MAX_NBR_PERIPHERALS)
