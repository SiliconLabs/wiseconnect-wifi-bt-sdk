...
#define DEVICE_IP 0x650AA8C0 //"192.168.10.101" //0x650AA8C0

//! IP address of Gateway
//! E.g: 0x010AA8C0 == 192.168.10.1
#define GATEWAY 0x010AA8C0 //"192.168.10.1" //0x010AA8C0

//! IP address of netmask
//! E.g: 0x00FFFFFF == 255.255.255.0
#define NETMASK 0x00FFFFFF //"255.255.255.0" //0x00FFFFFF
...
uint8_t ip_addr      = DEVICE_IP;
uint8_t network_mask = NETMASK;
...
int32_t rsi_socket_select()
{
  ...
  if (status != RSI_SUCCESS) {
    ...
    LOG_PRINT("wireless_init success\n");
  }
  //! Send feature frame
    ...
    switch (rsi_wlan_app_cb.state) {
      ...
      case RSI_WLAN_IPCONFIG_DONE_STATE: {
        ...
        for (count = 0; count < MAX_SOCK; count++) {
          //! Create socket
          server_socket[count] = rsi_socket(AF_INET, SOCK_DGRAM, 0);
          if (server_socket[count] < 0) {
            status = rsi_wlan_get_status();
            LOG_PRINT("\r\nSocket Create Failed, Error Code : 0x%lX\r\n", status);
            break;
          } else {
            LOG_PRINT("\n socket_create success for %d socket", count);
          }
      ...
      case RSI_WAIT_FOR_SELECT_CONFIRM: {
        memset(&read_fds, 0, sizeof(rsi_fd_set));
        for (count = 0; count < MAX_SOCK; count++) {
          if (!(rsi_getsockopt(server_socket[count],
                               SOL_SOCKET,
                               SO_CHECK_CONNECTED_STATE,
                               NULL,
                               (rsi_socklen_t)NULL))) {
            RSI_FD_SET(server_socket[count], &read_fds);
          }

          timeout.tv_sec = 1;
          status         = rsi_select(server_socket[count] + 1, &read_fds, NULL, NULL, &timeout, NULL);
          if (status <= 0) {
          } else {
            //! update wlan application state
      ...
      case RSI_WLAN_DATA_RECEIVE_STATE: {
        ...
        {
          for (count = 0; count < MAX_SOCK; count++) {
            if (RSI_FD_ISSET(server_socket[count], &read_fds)) {
              //! Receive data on socket
              status = rsi_recvfrom(server_socket[count],
                                    recv_buffer,
                                    recv_size,
                                    0,
                                    (struct rsi_sockaddr *)&client_addr[count],
                                    &addr_size);
              if (status < 0) {
                status = rsi_wlan_get_status();
                LOG_PRINT("\n data reception failed for %d socket\n", server_socket[count]);
                if (status == RSI_RX_BUFFER_CHECK) {
                  LOG_PRINT("\n failure status for %d socket:%X\n", server_socket[count], status);
                  break;
                }
              } else if (status > 0) {
                LOG_PRINT("\n data received on socket:%d\n", server_socket[count]);
              }
            }
