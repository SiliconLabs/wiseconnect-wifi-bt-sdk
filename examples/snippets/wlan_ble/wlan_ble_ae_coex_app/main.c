...
#define RSI_WLAN_TCP_SERVER_TASK_PRIORITY 1
...
#define RSI_WLAN_TCP_SERVER_TASK_STACK_SIZE (512 * 2)

rsi_semaphore_handle_t ui_task_sem;
rsi_semaphore_handle_t wlan_tcp_server_sem;
...
rsi_task_handle_t wlan_tcp_server_task_handle = NULL;
#ifdef FW_LOGGING_ENABLE
...
extern int32_t rsi_wlan_tcp_server_app(void);
...
int main(void)
{
  ...
  rsi_semaphore_create(&wlan_tcp_server_sem, 0);
#else
  _assert("NEED OS");
#endif
  ...
  rsi_task_create((void *)rsi_ble_main_app_task,
                  ...
                  &ble_task_handle);
#if WLAN_TASK_ENABLE
  rsi_task_create((void *)rsi_wlan_tcp_server_app,
                  (uint8_t *)"wlan_tcp_server_app_task",
                  RSI_WLAN_TCP_SERVER_TASK_STACK_SIZE,
                  NULL,
                  RSI_WLAN_TCP_SERVER_TASK_PRIORITY,
