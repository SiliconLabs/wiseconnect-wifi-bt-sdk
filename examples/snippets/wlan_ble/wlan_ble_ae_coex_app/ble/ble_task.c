...
extern rsi_semaphore_handle_t wlan_tcp_server_sem;
...
#if (WLAN_TASK_ENABLE && RSI_WITH_OS)
  rsi_semaphore_post(&wlan_tcp_server_sem);
#endif

#if (!WLAN_TASK_ENABLE && ENABLE_POWER_SAVE)
...
int32_t rsi_ble_dual_role(void)
{
  ...
  //! This should be vary from one device to other, Present SiWx917 dont have a support of FIXED IRK on every Reset
  ...
  if (BLE_AE_ADV_EVNT_PROP_SET_1 & BLE_LEGACY_ADV) {
    ...
    strncpy((char *)adv, RSI_BLE_AE_ADV_DATA, BLE_AE_ADV_DATA_LEN);
  }
  ...
  if (status != RSI_SUCCESS) {
    ...
    LOG_PRINT("Max number of supported Adv sets are %x  \n", rsi_app_resp_max_no_of_supp_adv_sets);
  }
